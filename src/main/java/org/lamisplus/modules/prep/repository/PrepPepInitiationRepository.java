...
public interface PrepPepInitiationRepository extends JpaRepository<PrepPepInitiation, Long>, JpaSpecificationExecutor<PrepPepInitiation> {
    ...
    // ─── HTS-encounter-driven Patient tab ─────────────────────────────────────
    // Pulls patients whose latest non-archived hts_encounter has a confirmatoryResult
    // of HTS_RESULT_HIV_NEGATIVE and a hivEarlyDetectResult in the two acute-infection
    // codes. All status / eligibility / commencement / interruption logic mirrors
    // findAllPersonPrepAndStatus so the UI status column behaves identically.
    String HTS_PATIENT_BASE_SELECT =
    ...
    String HTS_PATIENT_WHERE_FILTERS =
            ...
            "AND hts.observation->>'" + HtsObservationKeys.KEY_CONFIRMATORY_HIV_TEST + "' = '"
                    + HtsObservationKeys.CONFIRMATORY_HIV_NEGATIVE + "'\n" +
    ...
    @Query(value =
            HTS_PATIENT_BASE_SELECT +
            HTS_PATIENT_FROM +
            HTS_PATIENT_WHERE_FILTERS +
            HTS_PATIENT_GROUP_BY +
            ...
            countQuery =
                    ...
                    HTS_PATIENT_WHERE_FILTERS,
            nativeQuery = true)
    Page<PrepHtsPatient> findAllHtsEncounterPatientAndStatus(Boolean archived, Long facilityId, Pageable pageable);
    ...
    @Query(value =
            ...
            countQuery =
                    ...
                    "     OR hts.client_code ILIKE ?3)",
            nativeQuery = true)
    Page<PrepHtsPatient> findAllHtsEncounterPatientAndStatusBySearchParam(Boolean archived, Long facilityId, String search, Pageable pageable);
