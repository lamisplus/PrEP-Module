...
//            "he.date_confirmed_hiv, pet.id ORDER BY pet.date_created DESC NULLS LAST", nativeQuery = true)
    ...
    @Query(value = "SELECT \n" +
            ...
            "    LEFT JOIN (\n" +
            "        SELECT \n" +
            "            COUNT(el.person_uuid) AS eligibility_count,\n" +
            "            el.person_uuid\n" +
            "        FROM \n" +
            "            prep_eligibility el\n" +
            "        WHERE \n" +
            ...
            "        INNER JOIN (\n" +
            "            SELECT \n" +
            "                DISTINCT MAX(pc.encounter_date) AS encounter_date,\n" +
            "                pc.person_uuid\n" +
            "            FROM \n" +
            "                prep_clinic pc\n" +
            "            GROUP BY \n" +
            ...
            "            pc.archived = ?1\n" +
            "        GROUP BY \n" +
            "            pc.person_uuid, pc.duration, pc.previous_prep_status, pc.visit_type, pc.prep_type, status\n" +
            "    ) prepc ON prepc.person_uuid = p.uuid\n" +
            "    LEFT JOIN (\n" +
            "        SELECT \n" +
            "            pi.id,\n" +
            ...
            "        INNER JOIN (\n" +
            "            SELECT \n" +
            "                DISTINCT pi.person_uuid,\n" +
            "                MAX(pi.interruption_date) AS interruption_date\n" +
            "            FROM \n" +
            "                prep_interruption pi\n" +
            "            WHERE \n" +
            "                pi.archived = ?1\n" +
            ...
            "        INNER JOIN (\n" +
            "            SELECT \n" +
            "                DISTINCT MAX(el.visit_date) AS max_date,\n" +
            "                el.person_uuid\n" +
            "            FROM \n" +
            "                prep_eligibility el\n" +
            "            WHERE \n" +
            ...
            "        AND p.uuid = ?3\n" +
            "    GROUP BY \n" +
            "        prepi.interruption_date,\n" +
            "        prepc.encounter_date,\n" +
            "        bac.display,\n" +
            "        he.person_uuid,\n" +
            ...
            "        p.date_of_birth,\n" +
            "        prepc.status,\n" +
            "        prepc.previous_prep_status,\n" +
            "        prepi.interruption_type,\n" +
            "        prepc.visit_type,\n" +
            "        prepc.prep_type,\n" +
            "        prepc.duration,\n" +
            "        pet.id\n" +
            "    ORDER BY \n" +
            "        pet.date_created DESC NULLS LAST", nativeQuery = true)
