...
//            "he.date_confirmed_hiv, pet.id ORDER BY pet.date_created DESC NULLS LAST", nativeQuery = true)
    ...
    @Query(value = " SELECT DISTINCT ON (p.hospital_number) \n" +
            ...
            "        MAX(pc.encounter_date) AS encounter_date, pc.duration, pc.visit_type AS visit_type,\n" +
            ...
            "    WHERE pc.archived = ?1 \n" +
            "    GROUP BY pc.person_uuid, pc.duration, pc.visit_type, status\n" +
            ...
            "WHERE p.archived = ?1 AND p.facility_id = ?2 AND he.person_uuid IS NULL\n" +
            "AND (el_max.HIVResultAtVisit NOT ILIKE '%Positive%' OR el_max.HIVResultAtVisit IS NULL)\n" +
            "GROUP BY prepi.interruption_date, prepc.encounter_date, bac.display, el_max.HIVResultAtVisit,\n" +
            ...
            "    pet.date_created, p.other_name, p.hospital_number, p.date_of_birth, prepc.status,\n" +
            "    he.person_uuid, he.date_confirmed_hiv, pet.id, prepc.visit_type\n" +
