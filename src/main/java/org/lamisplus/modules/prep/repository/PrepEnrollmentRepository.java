...
//            "he.date_confirmed_hiv, pet.id ORDER BY pet.date_created DESC NULLS LAST", nativeQuery = true)
    ...
    @Query(value = "SELECT DISTINCT ON (p.hospital_number) el_max.HIVResultAtVisit, pet.date_created, p.date_of_registration AS dateOfRegistration, " +
            ...
            "   MAX(pc.encounter_date) as encounter_date, pc.duration, pc.visit_type, " +
            "   pc.prep_type, pc.previous_prep_status, " +
            "   (CASE WHEN (pc.encounter_date + pc.duration) > CAST(NOW() AS DATE) THEN 'Active' " +
            "       ELSE 'Defaulted' END) status " +
            "   FROM prep_clinic pc " +
            "   INNER JOIN ( " +
            "       SELECT DISTINCT MAX(pc.encounter_date) encounter_date, pc.person_uuid " +
            "       FROM prep_clinic pc GROUP BY pc.person_uuid " +
            "   ) max_p ON max_p.encounter_date = pc.encounter_date AND max_p.person_uuid = pc.person_uuid " +
            "   WHERE pc.archived = ?1 " +
            "   GROUP BY pc.person_uuid, pc.duration, pc.visit_type, pc.prep_type, pc.previous_prep_status, status " +
            ") prepc ON prepc.person_uuid = p.uuid " +
            ...
            "   FROM prep_eligibility el " +
            "   INNER JOIN (SELECT DISTINCT MAX(el.visit_date) as max_date, el.person_uuid " +
            "       FROM prep_eligibility el WHERE el.archived = ?1 " +
            "       GROUP BY person_uuid) pel ON pel.max_date = el.visit_date AND el.person_uuid = pel.person_uuid " +
            ") el_max ON el_max.person_uuid = p.uuid " +
            "WHERE p.archived = ?1 AND p.facility_id = ?2 AND p.uuid = ?3 " +
            "GROUP BY prepi.interruption_date, prepi.interruption_type, prepc.encounter_date, bac.display, " +
            "he.person_uuid, he.date_confirmed_hiv, el_max.HIVResultAtVisit, pet.date_created, " +
            "p.date_of_registration, prepc.commencementCount, el.eligibility_count, pet.created_by, " +
    ...
    @Query(value = "WITH json_data AS (\n" +
            "    SELECT\n" +
            "        CAST(j->>'address' AS text) AS address,\n" +
            "        CAST(j->>'id' AS integer) AS id,\n" +
            "        CAST(j->>'archived' AS integer) AS archived,\n" +
            "        CAST(j->>'sex' AS text) AS sex,\n" +
            "        CAST(j->>'age' AS integer) AS age,\n" +
            "        CAST(j->>'active' AS boolean) AS active,\n" +
            "        CAST(j->>'status' AS text) AS status,\n" +
            "        CAST(j->>'uuid' AS text) AS uuid,\n" +
            "        CAST(j->>'hospitalNumber' AS text) AS hospitalNumber,\n" +
            "        CAST(j->>'encounterDate' AS date) AS encounterDate,\n" +
            "        CAST(j->>'serviceCode' AS text) AS serviceCode,\n" +
            "        CAST(j->>'dateOfBirth' AS date) AS dateOfBirth,\n" +
            "        CAST(j->>'visitId' AS text) AS visitId,\n" +
            "        CAST(j->>'personUuid' AS text) AS personUuid,\n" +
            "        CAST(j->>'fullname' AS text) AS fullname,\n" +
            "        CAST(j->>'phone' AS text) AS phone,\n" +
            "        CAST(j->>'biometric' AS text) AS biometric,\n" +
            "        CAST(j->>'encounterId' AS integer) AS encounterId\n" +
            "    FROM json_array_elements(CAST(?3 AS json)) AS j\n" +
            "),\n" +
            "prep_data AS (\n" +
            ...
            "        SELECT\n" +
            "            pc.person_uuid,\n" +
            "            COUNT(pc.person_uuid) AS commencementCount,\n" +
            "            MAX(pc.encounter_date) AS encounter_date,\n" +
            "            pc.duration,\n" +
            "            pc.visit_type AS visit_type,\n" +
            "            pc.prep_type AS prep_type,\n" +
            "            pc.previous_prep_status AS previous_prep_status,\n" +
            "            (CASE WHEN (pc.encounter_date + pc.duration) > CAST(NOW() AS DATE) THEN 'Active' ELSE 'Defaulted' END) AS status\n" +
            "        FROM prep_clinic pc\n" +
            "        INNER JOIN (\n" +
            "            SELECT DISTINCT MAX(pc.encounter_date) AS encounter_date, pc.person_uuid\n" +
            ...
            "    jd.id AS json_id,\n" +
            "    jd.active,\n" +
            "    jd.status AS json_status,\n" +
            "    jd.uuid AS json_uuid,\n" +
            "    jd.encounterDate,\n" +
            "    jd.serviceCode,\n" +
            "    jd.visitId,\n" +
            "    jd.phone,\n" +
            ...
            "    jd.fullname\n" +
            "FROM prep_data pd\n" +
            "JOIN json_data jd ON jd.hospitalNumber = pd.hospitalNumber", nativeQuery = true)
    Page<PrepClient> findAllPocPersonPrepAndStatus(Integer archived, Long facilityId, String jsonData, Pageable pageable);
