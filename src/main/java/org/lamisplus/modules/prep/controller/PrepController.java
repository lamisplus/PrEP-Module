...
public class PrepController {
    ...
    @GetMapping(PREP_URL_VERSION_ONE + "/poc/persons")
    @ApiOperation("Get Prep Persons with optimized api")
    public ResponseEntity<PageDTO> getAllPocPersons(@RequestBody(required = false) Object jsonData,
                                                    @RequestParam(required = false, defaultValue = "*") String searchValue,
                                                    @RequestParam(required = false, defaultValue = "20") int pageSize,
                                                    @RequestParam(required = false, defaultValue = "0") int pageNo) {
        Page<PrepClient> page = prepService.getPocPrepClients(jsonData, searchValue, pageNo, pageSize);
        return new ResponseEntity<>(PaginationUtil.generatePagination(page, page.getContent()), HttpStatus.OK);
    }
