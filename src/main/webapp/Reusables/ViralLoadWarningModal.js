...
// App colour scheme. This is an informational warning (not a hard block like the
// HTS modal), so it uses amber for the header and the light blue dismiss action.
...
export const NO_VIRAL_LOAD_MESSAGE =
  ...
  "used to assess their PEP/PrEP outcome. This does not block any action — " +
  "please ensure a viral load result is captured when available.";

/**
 * Non-blocking warning shown on the patient dashboard when the latest viral
 * load lookup returns no record ({ viralLoad: null, viralLoadResult: null }).
 * Mirrors the HTS warning modal's look but is dismissible — VL is informational,
 * never a hard requirement. A missing VL is NOT treated as "Target Detected"; it
 * simply means "no record found".
 */
const ViralLoadWarningModal = ({ isOpen, onClose, message }) => (
  <Modal isOpen={isOpen} centered toggle={onClose}>
    <ModalHeader style={{ backgroundColor: SCHEME_AMBER }} toggle={onClose}>
